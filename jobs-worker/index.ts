...
async function runJob(job:ClaimedJob):Promise<Record<string,unknown>>{
  ...
  switch(job.type){
    ...
    case"analyze_competitor_content":return analyzeCreatorContent(admin,job.workspace_id,String(job.payload.contentId??""),job.payload);
    ...
    case"fetch_transcript":return fetchAndPersistTranscript(admin,job.workspace_id,String(job.payload.contentId??""));
...
async function heartbeat(job:ClaimedJob){
  await Promise.all([
    ...
    `,[workerId,job.id,{pid:process.pid}]),
  ]);
}
...
async function resumeConfiguredBlocked(){
  ...
  const dependencies=[];
  if(dependencyConfigured("ai_provider"))dependencies.push("ai_provider");
  if(dependencyConfigured("transcript_provider"))dependencies.push("transcript_provider");
...
async function loop(){
  ...
  while(!stopping){
    ...
    if(!job){
      await pool.query(`update public.worker_heartbeats set last_seen_at=now(),current_job_id=null where worker_id=$1`,[workerId]);
