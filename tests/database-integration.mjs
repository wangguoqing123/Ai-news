...
const v8=fs.readFileSync(new URL("../supabase/migrations/202608190001_signal_desk_metadata_first.sql",import.meta.url),"utf8");
const v9=fs.readFileSync(new URL("../supabase/migrations/202608190002_signal_desk_cluster_digest_path.sql",import.meta.url),"utf8");
...
try {
  ...
  assert.equal((await client.query("select count(*)::int as count from signal_desk_migrations")).rows[0].count,8);
  ...
  const translation=(await client.query("insert into content_translations(workspace_id,content_id,target_language,translated_title,translated_summary,input_hash,status) values($1,$2,'zh-CN','中文标题','中文摘要','translation-1','ready') returning id",[workspaceOne,content.id])).rows[0];await client.query("select public.activate_content_translation($1,$2,$3,'zh-CN')",[workspaceOne,content.id,translation.id]);assert.equal((await client.query("select translated_title from content_translations where content_id=$1 and is_current=true",[content.id])).rows[0].translated_title,"中文标题");
  await client.query("insert into content_processing_requests(workspace_id,content_id,requested_by,status) values($1,$2,$3,'queued')",[workspaceOne,content.id,userOne]);await assert.rejects(()=>client.query("insert into content_processing_requests(workspace_id,content_id,requested_by,status) values($1,$2,$3,'fetching_transcript')",[workspaceOne,content.id,userOne]),/duplicate key/i);await client.query("update content_processing_requests set status='ready',completed_at=now() where content_id=$1",[content.id]);await client.query("insert into content_processing_requests(workspace_id,content_id,requested_by,status) values($1,$2,$3,'queued')",[workspaceOne,content.id,userOne]);
  const clusterSource=(await client.query("insert into sources(workspace_id,type,external_id,name) values($1,'aihot','cluster-test','Cluster Test') returning id",[workspaceOne])).rows[0];const clusterContents=(await client.query("insert into content_items(workspace_id,source_id,external_id,content_type,title,summary,published_at,signal_score) values($1,$2,'cluster-1','article','OpenAI 发布模型','事实一',now()-interval '2 hours',70),($1,$2,'cluster-2','article','OpenAI 模型正式发布','事实二',now()-interval '1 hour',80),($1,$2,'cluster-new','article','OpenAI 新模型发布','事实三',now(),90) returning id,external_id",[workspaceOne,clusterSource.id])).rows;const byExternal=Object.fromEntries(clusterContents.map(row=>[row.external_id,row.id]));const clusterA=(await client.query("insert into event_clusters(workspace_id,title,status,analysis_input_hash) values($1,'旧簇 A','active','old-a') returning id",[workspaceOne])).rows[0];const clusterB=(await client.query("insert into event_clusters(workspace_id,title,status,analysis_input_hash) values($1,'旧簇 B','active','old-b') returning id",[workspaceOne])).rows[0];await client.query("insert into event_cluster_items(workspace_id,cluster_id,content_id) values($1,$2,$3),($1,$4,$5)",[workspaceOne,clusterA.id,byExternal["cluster-1"],clusterB.id,byExternal["cluster-2"]]);await client.query("insert into event_analyses(workspace_id,cluster_id,status,input_hash,prompt_version,profile_version,analysis_version,is_current) values($1,$2,'ready','old-a','p',1,'v',true),($1,$3,'ready','old-b','p',1,'v',true)",[workspaceOne,clusterA.id,clusterB.id]);const reconciled=(await client.query("select public.reconcile_event_cluster_group($1,$2,$3) as id",[workspaceOne,[byExternal["cluster-new"],byExternal["cluster-1"],byExternal["cluster-2"]],{title:"OpenAI 新模型发布",summary:"合并事实",first_seen_at:new Date(Date.now()-7200000).toISOString(),last_seen_at:new Date().toISOString(),facts:[],confidence:70,importance:90,timeliness:100,topics:["OpenAI"],analysis_input_hash:"candidate"}])).rows[0].id;assert.equal((await client.query("select count(*)::int as count from event_clusters where id=any($1) and status='active'",[[clusterA.id,clusterB.id]])).rows[0].count,1);assert.equal((await client.query("select count(*)::int as count from event_cluster_items where cluster_id=$1",[reconciled])).rows[0].count,3);assert.equal((await client.query("select count(distinct cluster_id)::int as count from event_cluster_items where content_id=any($1)",[[byExternal["cluster-1"],byExternal["cluster-2"],byExternal["cluster-new"]]])).rows[0].count,1);assert.equal((await client.query("select count(*)::int as count from event_analyses where cluster_id=any($1) and is_current=true",[[clusterA.id,clusterB.id]])).rows[0].count,0);assert.ok((await client.query("select analysis_input_hash from event_clusters where id=$1",[reconciled])).rows[0].analysis_input_hash.length===64);
  ...
  assert.equal((await client.query("select count(*)::int as count from sources")).rows[0].count,2);
  ...
  await client.query("insert into daily_briefs(workspace_id,brief_date,timezone,status,summary,completed_at) values($1,current_date,'Asia/Shanghai','final',$2,now())",[workspaceOne,{entries:[{kind:"event",sourceId:content.id}]}]);
  ...
  console.log("database integration: nine migrations, metadata/deep idempotency, cluster merge, ordered current switches, worker ownership, RLS, audit and schedules passed");
} finally {
