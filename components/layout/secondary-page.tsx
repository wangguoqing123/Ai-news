...
export function SecondaryPage({kind}:{kind:keyof typeof configs}){
  ...
  return <div className="v2-page">
    <header className="v2-page-header"><div><div className="v2-date">二级入口</div><h1>{config.title}</h1><p>{config.description}</p></div><div className="v2-header-actions">{kind==="jobs"&&<button onClick={()=>void requeueBlocked()}>恢复 blocked</button>}<button className="v2-refresh-button"onClick={()=>void load()}><RefreshCw size={15}/>刷新</button></div></header>
