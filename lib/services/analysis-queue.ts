...
export async function enqueueContentAnalysis(admin:SupabaseClient,input:{workspaceId:string;contentId:string;type?:"analyze_creator_content"|"analyze_competitor_content";contentHash?:string|null;priority?:number;requeueExisting?:boolean;supersedePending?:boolean}){
  ...
  const job=await enqueueJob(admin,{workspaceId:input.workspaceId,type,idempotencyKey,payload:{contentId:input.contentId,contentHash:baseHash,inputHash,promptVersion:CREATOR_PROMPT_VERSION,profileVersion,analysisVersion:ANALYSIS_VERSION},priority:input.priority??60,requeueExisting:input.requeueExisting});if(job.status!=="cancelled")return job;const restored=await admin.from("jobs").update({status:"queued",run_at:new Date().toISOString(),attempt:0,result:null,error:null,locked_at:null,locked_by:null,lease_expires_at:null,heartbeat_at:null}).eq("id",job.id).select("id,type,status,priority,payload,idempotency_key,created_at").single();if(restored.error||!restored.data)throw new Error(restored.error?.message??"恢复分析任务失败");return restored.data;
}
