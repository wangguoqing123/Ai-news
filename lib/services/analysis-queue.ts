...
export async function enqueueContentAnalysis(admin:SupabaseClient,input:{workspaceId:string;contentId:string;type?:"analyze_creator_content"|"analyze_competitor_content";contentHash?:string|null;priority?:number;requeueExisting?:boolean;supersedePending?:boolean}){
  ...
  const pending=await admin.from("jobs").select("id,payload").eq("workspace_id",input.workspaceId).eq("type",type).in("status",["queued","blocked"]).contains("payload",{contentId:input.contentId});if(pending.error)throw new Error(pending.error.message);const obsoleteIds=(pending.data??[]).filter(item=>input.requeueExisting||record(item.payload).inputHash!==inputHash).map(item=>item.id);if(obsoleteIds.length){const superseded=await admin.from("jobs").update({status:"cancelled",result:{reason:"superseded_by_new_analysis_input",inputHash},locked_at:null,locked_by:null,lease_expires_at:null,heartbeat_at:null}).in("id",obsoleteIds);if(superseded.error)throw new Error(superseded.error.message);}
