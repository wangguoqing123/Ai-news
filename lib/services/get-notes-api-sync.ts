...
export async function syncGetNotesApi(admin:SupabaseClient,workspaceId:string,options:{maxPages?:number}={}){
  ...
  const connector=new GetNotesConnector();const source=await ensureSource(admin,{workspaceId,type:"get_notes",externalId:knowledgeBaseId,name:"Get 笔记 · 竞品内容",metadata:{mode:"api",knowledgeBaseId}});const runId=await startSyncRun(admin,{workspaceId,sourceId:source.id});
  let cursor:string|null=null,pages=0,fetched=0,normalized=0,duplicates=0;
  try{
    do{const page=await connector.fetchPage({config,cursor});pages+=1;fetched+=page.items.length;for(const raw of page.items){const item=connector.normalizeWithMapping(raw,mapping);item.sourceMetadata={...item.sourceMetadata,knowledgeBaseId,provenance:"verified_live",interactionStatus:item.sourceMetadata.interactionAvailable?"available":"unavailable"};const persisted=await persistNormalizedContent(admin,{workspaceId,sourceId:source.id,sourceType:"get_notes",syncRunId:runId,raw,normalized:item});normalized+=1;if(persisted.duplicateOfId)duplicates+=1;if(persisted.shouldAnalyze)await enqueueContentAnalysis(admin,{workspaceId,contentId:persisted.id,type:"analyze_competitor_content",contentHash:persisted.contentHash,priority:55});}cursor=page.hasMore?page.nextCursor??null:null;}while(cursor && pages<(options.maxPages ?? 50));
